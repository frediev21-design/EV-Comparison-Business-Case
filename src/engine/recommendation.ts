...
export function generateRecommendation(
  ...
  return `Based on the assumptions entered, upgrading to the ${selected.name} ${savingDirection} indicative monthly cash flow by approximately ${absMonthly} per month while providing predictable ownership expenses, warranty protection, and lower dependence on ${input.current.fuelType === "diesel" ? "diesel" : "fossil fuels"}. Trade equity of R${tradeIn.tradeEquity.toLocaleString()} combined with R${tradeIn.additionalCash.toLocaleString()} additional deposit leaves R${tradeIn.amountFinanced.toLocaleString()} to finance at ${selected.interestRate}% over ${selected.financeTermMonths} months.${solarNote}${warrantyNote} Over 10 years, the net ownership cost delta (finance, running costs, and resale) is R${Math.abs(kpis.tenYearSaving).toLocaleString()}, with indicative payback of ${kpis.paybackMonths > 0 ? `${kpis.paybackMonths} months` : "N/A"} on net finance. These figures are estimates only and not financial advice.`;
}
